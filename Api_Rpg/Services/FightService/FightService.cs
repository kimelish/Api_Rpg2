...
namespace Api_Rpg.Services.FightService
{
    public class FightService : IFightService
    {
        ...
        public FightService(IMapper mapper, DataContext dataContext)
        {
            _mapper = mapper;
        ...
        private int DoSkillAttack(Character attacker, Character opponent, CharacterSkill characterSkill)
        {
            int damage = characterSkill.Skill.Damage + (new Random().Next(attacker.Intelligence));
            damage -= new Random().Next(opponent.Defence);
            if (damage > 0) opponent.HitPoints -= (int)damage;
            return damage;
        }
